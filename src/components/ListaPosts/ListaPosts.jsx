...
const ListaPosts = ({ url }) => {
  ...
  useEffect(() => {
    async function getPosts() {
      try {
        ...
        /* solição 3 */
        /* const resposta = await fetch(`${serverApi}/${props.url != undefined ? props.url : "posts"}`); */
    ...
    getPosts();
  }, [url]); /* É necessário indicar a url como dependência pois ela
  ...
  if (loading) {
    return <LoadingPacman carregando={"Carregando Posts..."} />;
  }
  /* Sobre o use Effect:
  ...
  if (posts.length == 0) {
    return <h2 className={estilos.semPost}>Não há Posts...</h2>;
  } else {
    return (
